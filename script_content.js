...
var MLBITE_URL = 'https://mlbite.to/';
...
function openGlobalStats(teamName) {
    ...
    if (teamName) {
        ...
        content.innerHTML = '<div style="text-align:center;color:var(--muted);margin-top:20px;">Chargement des statistiques pour ' + esc(teamName) + '...</div>';

        fetchTeamStats(teamName);
    } else {
        title.textContent = 'Scores & Stats';
        backBtn.style.display = 'none';
        content.innerHTML = '<div style="text-align:center;color:var(--muted);margin-top:20px;">Cliquez sur une équipe dans l\'onglet FAVORIS pour voir ses statistiques.</div>';
    }
}

function gstatsGoBack() {
    openGlobalStats();
}
...
function fetchTeamStats(teamName) {
    ...
    if (logoUrl) {
        html += '<div style="text-align:center;margin-bottom:16px;"><img src="'+esc(logoUrl)+'" style="max-width:100px;max-height:100px;object-fit:contain;"></div>';
    }

    html += '<h4 style="color:#fff;margin-bottom:8px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:4px;">Matchs Associés (Aujourd\'hui)</h4>';

    if (teamMatches.length > 0) {
        teamMatches.forEach(function(m) {
            html += '<div style="background:rgba(255,255,255,0.05);padding:12px;border-radius:8px;margin-bottom:8px;display:flex;flex-direction:column;gap:4px;cursor:pointer;" onclick="openMod(S.matches.find(x=>x.id===\''+escJs(m.id)+'\'))">';
            html += '<div style="font-size:12px;color:var(--muted);">'+esc(m.league)+' • '+esc(m.startTime)+'</div>';
            html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
            html += '<span style="font-weight:bold;color:#fff;">'+esc(m.homeTeam)+'</span>';
            var hScore = m.score ? m.score[0] : '-';
            html += '<span style="font-weight:bold;color:var(--accent);">'+hScore+'</span>';
            html += '</div>';
            html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
            html += '<span style="font-weight:bold;color:#fff;">'+esc(m.awayTeam)+'</span>';
            var aScore = m.score ? m.score[1] : '-';
            html += '<span style="font-weight:bold;color:var(--accent);">'+aScore+'</span>';
            html += '</div>';
            if (m.status === 'live') {
                html += '<div style="color:var(--red);font-size:12px;font-weight:bold;margin-top:4px;">🔴 '+(m.minute?esc(m.minute)+'\'' : 'En Direct')+'</div>';
            }
            html += '</div>';
        });
    } else {
        html += '<div style="color:var(--muted);font-size:13px;">Aucun match aujourd\'hui pour cette équipe.</div>';
    }

    html += '</div>';
    html += '<div style="color:var(--muted);font-size:12px;text-align:center;margin-top:20px;padding:10px;background:rgba(0,0,0,0.3);border-radius:8px;">Statistiques détaillées à venir via API-Sports / ESPN</div>';
...
function normName(n) {
  ...
  lower = lower.replace(/\b(fc|afc|sc|cf|racing|de)\b/g, '').trim();
    ...
    if (group.info.leagueId) {
      promises.push(
        fetchApiSportsFixtures(group.info, today).then(function(apiData) {
          ...
          var normalizedApiData = apiData.map(function(f) {
            return {
              fixture: f,
              home: normName(f.teams.home.name),
              away: normName(f.teams.away.name)
            };
          });
          ...
          group.matches.forEach(function(m) {
            ...
            var matchedApiFixture = normalizedApiData.find(function(nf) {
              // Simple check if one contains the other + fuzzy matching
              return isMatch(mHome, nf.home) && isMatch(mAway, nf.away);
            });

            if (matchedApiFixture) {
              updateMatchDataFromApi(m, matchedApiFixture.fixture, group.info.sport);
            }
          });
        })
      );
...
function syncMatchesWithEspn(matches) {
  var today = new Date();
  ...
  matches.forEach(function(m) {
    var l = m.league.toLowerCase();
    var espnPath = null;
    for(var key in ESPN_LEAGUES) {
      if(l.indexOf(key) >= 0) {
        espnPath = ESPN_LEAGUES[key];
        break;
      }
    }
    if(espnPath) {
      if(!leagueMatches[espnPath]) leagueMatches[espnPath] = [];
      leagueMatches[espnPath].push(m);
    }
  });

  Object.keys(leagueMatches).forEach(function(path) {
    var group = leagueMatches[path];
      ...
      promises.push(
        fetchEspnSchedule(path, dateStr).then(function(data) {
          ...
          data.events.forEach(function(ev) {
            ...
            var eHome = normName(home.team.name);
            var eAway = normName(away.team.name);

            group.forEach(function(m) {
              var mHome = normName(m.homeTeam);
              var mAway = normName(m.awayTeam);

              if(isMatch(mHome, eHome) && isMatch(mAway, eAway)) {

                var d = new Date(ev.date);
                m.startTime = getEstTimeStrFromDate(d);

                var st = ev.status.type.state; // 'pre', 'in', 'post'
                if(st === 'pre') {
                  m.status = 'upcoming';
                } else if(st === 'in') {
                  m.status = 'live';
                  m.minute = ev.status.displayClock;
                  m.score = [parseInt(home.score)||0, parseInt(away.score)||0];
                } else if(st === 'post') {
                  m.status = 'finished';
                  m.score = [parseInt(home.score)||0, parseInt(away.score)||0];
                  m.minute = 'FT';
                ...
                // Set logos from ESPN
                if (home.records && home.records.length > 0) {
                   m.homeRecord = home.records[0].summary;
                }
                if (away.records && away.records.length > 0) {
                   m.awayRecord = away.records[0].summary;
                }
                if(home.team.logo) {
                    m.homeLogo = home.team.logo;
                    cacheLogo(m.homeTeam, m.homeLogo);
                }
                if(away.team.logo) {
                    m.awayLogo = away.team.logo;
                    cacheLogo(m.awayTeam, m.awayLogo);
                }
              }
            });
          });
...
try {
    ...
    if(stored) logoCache = JSON.parse(stored);
} catch(e) {}
...
function scrapeMatchFlux(m){
  ...
  return Promise.race([
    ...
    [].forEach.call(iframes, function(ifr) {
        var src = ifr.src;
        if(src && src.indexOf('http') === 0 && src.indexOf('footybite') < 0 && src.indexOf('ads') < 0) {
            links.push({
                name: 'Lecteur direct',
                quality: 'HD',
                lang: 'MULTI',
    ...
    [].forEach.call(rows, function(row){
        ...
        if(input && input.value && input.value.indexOf('http') === 0) {
            ...
            for(var i=0; i<as.length; i++) {
                 if(as[i].href.indexOf('http')===0 && as[i].href.indexOf('footybite')<0) {
                     url = as[i].href;
                     break;
                 }
            }
        ...
        if(url && typeof url === 'string') {
            ...
            var channel = '';
            var thirdColText = tds[2] ? tds[2].textContent.trim() : '';
            var firstColText = tds[0] ? tds[0].textContent.trim() : '';

            if (thirdColText && thirdColText.length > 2 && thirdColText.length < 20 && !thirdColText.includes('Kbps')) {
                 channel = thirdColText;
            } else if (firstColText && firstColText.length > 2 && firstColText.length < 20) {
                 channel = firstColText;
            }

            if(channel) {
                channel = channel.replace(/\s+/g, ' ').trim();
                if (channel && channel.toLowerCase() !== 'sd' && channel.toLowerCase() !== 'hd' && !name.includes(channel)) {
                    name = channel + ' (' + name + ')';
                }
            }

            var lang = 'MULTI';
            if(rowText.indexOf('english') >= 0 || rowText.indexOf(' uk') >= 0) lang = 'ENGLISH';
            else if(rowText.indexOf('french') >= 0 || rowText.indexOf(' fr') >= 0) lang = 'FRENCH';
            else if(rowText.indexOf('spanish') >= 0 || rowText.indexOf('es') >= 0) lang = 'SPANISH';
            else if(rowText.indexOf('arabic') >= 0 || rowText.indexOf('ar') >= 0) lang = 'ARABIC';
    ...
    if(links.length===0){
       ...
       [].forEach.call(btns,function(btn){
          if(btn.tagName==='A' && btn.href){
             var url=btn.href;
             if(url && url.indexOf('http')===0 && url.indexOf('footybite')<0) {
                 var lowerUrl = url.toLowerCase();
    ...
    if(links.length===0) {
        ...
        [].forEach.call(elementsWithData, function(el) {
            ...
            if(url) {
                ...
                if(url.indexOf('http') === 0 && url.indexOf('footybite') < 0) {
                    var lowerUrl = url.toLowerCase();
                    if (!lowerUrl.includes('1xbet') && !lowerUrl.includes('bet365') && !lowerUrl.includes('ads') && lowerUrl.length >= 5) {
                        links.push({
                            name: 'Lecteur caché',
    ...
    // Limit to 20 streams maximum to prevent UI overload
    m.streamLinks = links.slice(0, 20);
...
function changeDate(dStr) {
  if(!dStr) return;
  var parts = dStr.split('-');
  TARGET_DATE = new Date(parts[0], parts[1] - 1, parts[2]);
  loadAll();
}

function applyFilter(f){
  S.filter=f;
  ['all','live','upcoming'].forEach(function(k){
    var el=document.getElementById('filter-'+k);
    if(el){
        el.style.borderColor = k===f ? 'var(--accent)' : 'var(--border2)';
        el.style.color = k===f ? 'var(--accent)' : 'var(--muted2)';
    }
  });
...
function getOriginalMatchId(id) {
    if (typeof id === 'string' && id.endsWith('_live_copy')) {
        return id.replace('_live_copy', '');
    }
    return id;
}
...
function buildEPG(matches){
  ...
  if (S.filter === 'all') {
      ...
      epgMatches.forEach(function(m) {
          if (m.status === 'live' && m.league !== 'Autres Flux') {
      ...
      leagues.forEach(function(lg) {
          ...
          if (!isCollapsed) {
              lg.matches.forEach(function(m) {
                  ...
                  if (tColorsH) { homeColor = tColorsH[0]; awayColor = tColorsH[1]; }
                  if (tColorsA && !tColorsH) { homeColor = tColorsA[1]; awayColor = tColorsA[0]; }
                  ...
                  } else if (userPrefs.cardColor === 'gradient-top') {
                      b.style.background = 'linear-gradient(to bottom, ' + homeColor + ' 0%, ' + awayColor + ' 100%)';
                  } else {
...
function renderFluxItem(s, i, m) {
    ...
    var addMvEv="addToMultivision('"+escJs(s.url||'#')+"','"+escJs(m.homeTeam)+' vs '+escJs(m.awayTeam)+"');closeMod();event.stopPropagation();event.preventDefault();";
    ...
    return '<div class="si">'
      ...
      +'<div class="si-s">Langue : '+(s.lang||'?')+' · '+(i===0?'Recommandé':'Alternatif')+'</div></div>'
...
function openMod(m,col){
  ...
  if(!m.streamsLoaded && m.matchUrl) {
      body.innerHTML='<div style="text-align:center;padding:20px;color:var(--muted2);">Chargement des streams... <span style="font-size: 0.8em; opacity: 0.5;">(Patientez)</span></div>';
      ...
      // We must show the modal right away so the user sees the "Loading" state
    ...
    if (m && m.streamLinks && m.streamLinks.length > 0) {
        ...
        sortedLinks.forEach(function(s) {
            ...
            btn.innerHTML = '<div style="font-size:16px;">' + (s.icon||QI[s.quality]||'📺') + '</div>' +
                            ...
                            '<div style="font-size:11px;color:var(--muted);">' + esc(dom) + ' · ' + (s.lang||'?') + '</div>' +
                            '</div>' +
...
function toggleMaximize(idx, event) {
    if (event) {
        event.stopPropagation();
        event.preventDefault();
    }
    // Convert to string for querySelector
    var idxStr = String(idx);
    var cell = document.querySelector('.mv-cell[data-index="' + idxStr + '"]');
    if (!cell) return;

    if (cell.classList.contains('mv-maximized')) {
        cell.classList.remove('mv-maximized');
        ...
        // Remove maximized from others first
        document.querySelectorAll('.mv-cell.mv-maximized').forEach(function(c) {
            c.classList.remove('mv-maximized');
        ...
        cell.classList.add('mv-maximized');
...
function toggleCrop(idx, event) {
    if (event) {
        event.stopPropagation();
        event.preventDefault();
    }
    mvFlux[idx].cropped = !mvFlux[idx].cropped;
    var iframe = document.getElementById('mv-iframe-' + idx);
    if (iframe) {
        if (mvFlux[idx].cropped) {
            iframe.style.transform = 'scale(1.15)';
        } else {
            iframe.style.transform = 'scale(1)'; } } saveMultivisionState();
}



/* ══ MULTIVISION MATCH SELECTOR ════════ */
function showMatchSelector(event) {
    if (event) {
        event.stopPropagation();
        event.preventDefault();
    }
    ...
    selector.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%, -50%);z-index:9999;background:rgba(20,20,20,0.95);border:1px solid rgba(255,255,255,0.2);border-radius:12px;padding:12px;backdrop-filter:blur(15px);display:flex;flex-direction:column;gap:8px;max-height:80vh;overflow-y:auto;box-shadow:0 15px 40px rgba(0,0,0,0.8);width:90%;max-width:320px;-webkit-overflow-scrolling:touch;';

    // Centered using position fixed

    var title = document.createElement('div');
    title.style.cssText = 'font-size:11px;font-weight:bold;color:var(--muted);text-transform:uppercase;margin-bottom:4px;padding:0 4px;';
    title.textContent = 'Ajouter un match';
    selector.appendChild(title);

    // Filter matches that have streams
    var availableMatches = S.matches.filter(function(m) {
        return m.streamsLoaded && m.streamLinks && m.streamLinks.length > 0;
    });
    ...
    if (availableMatches.length === 0) {
        var empty = document.createElement('div');
        empty.style.cssText = 'padding:10px;text-align:center;color:var(--muted);font-size:12px;';
        ...
        availableMatches.forEach(function(m) {
            ...
            if (alreadyIn) return; // Skip


            var btn = document.createElement('div');
            btn.style.cssText = 'display:flex;align-items:center;gap:10px;padding:12px;min-height:44px;border-radius:8px;cursor:pointer;transition:all 0.2s;background:rgba(255,255,255,0.05);';
            btn.onmouseenter = function() { this.style.background = 'rgba(255,255,255,0.1)'; };
            ...
            var thumb = '<div style="width:36px; height:36px; border-radius:4px; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; font-size:16px;">' + lgFlag(m.league) + '</div>';
            if(m.homeLogo) {
                thumb = '<div style="width:36px; height:36px; border-radius:4px; background:rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:center; overflow:hidden; padding:2px;flex-shrink:0;">'
            ...
            var score = m.score ? '<div style="font-size:13px; font-weight:bold; color:var(--accent); display:flex; flex-direction:column; align-items:center; justify-content:center; background:rgba(0,0,0,0.3); padding:4px 8px; border-radius:4px;flex-shrink:0;"><span>'+m.score[0]+'</span><span>'+m.score[1]+'</span></div>' : '';

            btn.innerHTML = thumb +
                '<div style="flex:1; overflow:hidden;">' +
                    '<div style="font-size:10px; color:var(--red); font-weight:bold; margin-bottom:2px;">' + (m.status === 'live' ? '🔴 '+(m.minute?esc(m.minute)+"'":'EN DIRECT') : esc(m.startTime)) + ' <span style="color:var(--muted); font-weight:normal; margin-left:4px;">'+m.flag+' '+esc(m.league)+'</span></div>' +
                    '<div style="font-size:12px; font-weight:bold; color:#fff; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;">'+esc(m.homeTeam)+'</div>' +
                    '<div style="font-size:12px; font-weight:bold; color:#fff; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;">'+esc(m.awayTeam)+'</div>' +
                '</div>' + score;
...
function toggleMultiviewPip() {
    ...
    if(mvc.classList.contains('mv-pip')) {
        ...
        mvc.style.cssText = 'position:fixed;top:60px;left:0;right:0;bottom:0;background:#000;z-index:90;display:flex;flex-direction:column;';
        ...
        if (isMobile) {
            ...
            mvc.style.cssText = 'position:fixed;top:70px;right:0;bottom:0;width:350px;background:#0a0a0c;z-index:90;display:flex;flex-direction:column;border-left:1px solid var(--border);box-shadow:-5px 0 20px rgba(0,0,0,0.5);';
            epg.style.display = 'flex';
...
function setupMultivisionUI() {
    ...
    mvContainer.style.cssText = 'position:fixed;top:60px;left:0;right:0;bottom:0;background:#000;z-index:90;display:none;flex-direction:column;';
    ...
    var mvToolbarHtml = '<span style="font-weight:bold;color:var(--text);"><span class="hide-pip">Mode </span>Multivision</span>'
      + '<div class="sp" style="flex:1;"></div>'
      + '<button class="btn g" onclick="showMatchSelector(event)" aria-label="Ajouter un match" title="Ajouter un match" style="padding: 4px 8px;">➕ <span class="hide-pip">Ajouter</span></button>'
      ...
      + '<button class="btn o" onclick="toggleMultiviewPip()" id="mv-pip-btn" aria-label="Réduire" title="Réduire" style="padding: 4px 8px;">◨ <span class="hide-pip">Réduire</span></button>'
      + '<button class="btn o" onclick="hideMultivision()" aria-label="Fermer le Multivision" title="Fermer le Multivision" style="padding: 4px 8px;">✕ <span class="hide-pip">Fermer</span></button>'
    ...
    exitTheaterBtn.onclick = toggleTheaterMode;
...
function toggleTheaterMode() {
    var mvc = document.getElementById('mv-container');
    var tb = document.getElementById('mv-toolbar');
    var exitBtn = document.getElementById('mv-exit-theater');
    var mainHdr = document.querySelector('.hdr');

    if(!mvc || !tb || !exitBtn || !mainHdr) return;

    if (mvc.classList.contains('theater-mode')) {
        mvc.classList.remove('theater-mode');
        mvc.style.top = '60px'; // Revert back under the main header
        tb.style.display = 'flex';
        mainHdr.style.display = 'flex'; // Show main header
        exitBtn.style.display = 'none';
        exitBtn.style.opacity = '0';
    } else {
        mvc.classList.add('theater-mode');
        mvc.style.top = '0px'; // Full window height
        tb.style.display = 'none'; // Hide multiview toolbar
        mainHdr.style.display = 'none'; // Hide main header
...
function saveMultivisionState() {
    try {
        localStorage.setItem('mv_state', JSON.stringify(mvFlux));
    } catch(e) {}
}
    ...
    try {
        ...
        if(saved) {
            ...
            if(Array.isArray(parsed) && parsed.length > 0) {
                mvFlux = parsed;
                // Automatically open multiview as PiP or Background if streams exist
                // To avoid interrupting the initial load, we just update the UI badge
                setTimeout(function() {
                    updateMultivisionLayout();
                }, 500);
            }
...
function setFocusFlux(idx) {
    if (idx === 0) return; // Already focused
    // Swap flux at idx with flux at 0
    var temp = mvFlux[0];
    mvFlux[0] = mvFlux[idx];
    mvFlux[idx] = temp;
...
function updateMultivisionLayout() {
    ...
    mvFlux.forEach(function(s, idx) {
        ...
        var controlsHtml = '<div style="display:flex;gap:6px;pointer-events:auto;background:rgba(0,0,0,0.3);padding:4px;border-radius:8px;backdrop-filter:blur(5px);">';

        if (s.mid) {
             controlsHtml += '<button title="Changer de flux" aria-label="Changer de flux" style="background:rgba(255,255,255,0.1);color:#fff;border:none;border-radius:4px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background 0.2s;" onmouseover="this.style.background=\'rgba(255,255,255,0.2)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.1)\'" onclick="showFluxSelector(' + idx + ', \'' + escJs(s.mid) + '\', event);event.stopPropagation();">🔄</button>';
        }
        controlsHtml += '<button title="Recharger" aria-label="Recharger" style="background:rgba(255,255,255,0.1);color:#fff;border:none;border-radius:4px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background 0.2s;" onmouseover="this.style.background=\'rgba(255,255,255,0.2)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.1)\'" onclick="var fr = document.getElementById(\'mv-iframe-' + idx + '\'); if(fr) { var sr = fr.src; fr.src = \'\'; setTimeout(function(){fr.src=sr;}, 100); }; event.stopPropagation();">↻</button>';
        controlsHtml += '<button title="Recadrer" aria-label="Recadrer" style="background:rgba(255,255,255,0.1);color:#fff;border:none;border-radius:4px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:background 0.2s;" onmouseover="this.style.background=\'rgba(255,255,255,0.2)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.1)\'" onclick="toggleCrop(' + idx + ', event);event.stopPropagation();">🔍</button>';
...
function openFlux(e, eu, en, mid){
  ...
  if(!pbg){
    ...
    pbg.innerHTML = '<div id="player-container" style="width:100%; height:100%; background:#000; display:flex; flex-direction:column; position:relative; overflow:hidden;">'
        ...
        sortedLinks.forEach(function(s, i) {
            ...
            div.innerHTML = '<div '+mainClickAttr+' '+hoverLogic+' style="flex:1;display:flex;align-items:center;gap:10px;padding:10px;cursor:'+cursorStyle+';">'
                ...
                +'<div style="flex:1; overflow:hidden;">'
                  ...
                  +'<div style="font-size:11px; color:var(--muted);">'+(s.lang||'?')+'</div>'
                +'</div>'
...
function openDbg(){
  var h='';
  h+='<div class="dbg-sect"><div class="dbg-sect-title">Statut</div><div class="dbg-code">';
  h+=S.log.length===0?'Aucun chargement lancé.':'Proxy: '+S.proxy+'\nMatches: '+S.matches.length+'\nHTML: '+S.raw.length+' chars';
  h+='</div></div>';

  if(S.matches.length>0){
    var errMatches = S.matches.filter(function(m) { return m.streamsLoaded && (!m.streamLinks || m.streamLinks.length===0 || m.streamLinks.length===1); });
    h+='<div class="dbg-sect"><div class="dbg-sect-title">Statut Flux Match par Match</div><div class="dbg-code" style="max-height:400px;">';
    S.matches.forEach(function(m){
        var st = m.streamsLoaded ? (m.streamLinks.length + ' streams trouvés') : 'En attente...';
        if(m.streamsLoaded && m.streamLinks.length===0) st = '<span style="color:var(--red)">0 flux trouvé</span>';
        h += '['+m.league+'] '+esc(m.homeTeam)+' vs '+esc(m.awayTeam)+'\n';
        h += 'URL: '+esc(m.matchUrl)+'\n';
        h += 'Flux: '+st+'\n\n';
    });
    h+='</div></div>';
  }
  ...
  if(S.log.length>0){
    h+='<div class="dbg-sect"><div class="dbg-sect-title">Log ('+S.log.length+')</div><div class="dbg-code">';
    S.log.forEach(function(e){h+=esc('['+e.l+'] '+e.v)+'\n\n';});
    h+='</div></div>';
  }
  if(S.raw){
    h+='<div class="dbg-sect"><div class="dbg-sect-title">HTML brut [6000]</div><div class="dbg-code">'+esc(S.raw.slice(0,6000))+'</div></div>';
  }
  if(S.matches.length>0){
    h+='<div class="dbg-sect"><div class="dbg-sect-title">Data Matchs ('+S.matches.length+')</div><div class="dbg-code">'+esc(JSON.stringify(S.matches.slice(0,10),null,2))+'</div></div>';
  }
  document.getElementById('dbgbody').innerHTML=h;
  document.getElementById('dbg').classList.add('open');
}
function closeDbg(){document.getElementById('dbg').classList.remove('open');}
...
function initPrefs() {
  ...
  if(userPrefs.bg === 'solid') {
    document.body.style.background = '#000';
  } else {
    document.body.style.background = '#000 radial-gradient(circle at 50% -20%, #1a1a24 0%, #000 70%)';
  }

  // Accent Color
  document.documentElement.style.setProperty('--accent', userPrefs.accent || '#0a84ff');

  // Btn Shape
  var br = '12px';
  if(userPrefs.btnShape === 'square') br = '4px';
  else if(userPrefs.btnShape === 'pill') br = '24px';
  document.documentElement.style.setProperty('--radius-btn', br);

  // Card Radius and Opacity
  var cr = '16px';
  if(userPrefs.btnShape === 'square') cr = '8px';
...
function installTampermonkey() {
    ...
    contentDiv.innerHTML = `
        ...
        <ol style="padding-left:20px;margin:0;display:flex;flex-direction:column;gap:8px;">
            <li>Installez l'extension <strong>Tampermonkey</strong> sur votre navigateur (<a href="https://www.tampermonkey.net/" target="_blank" style="color:var(--accent);">Lien</a>).</li>
            <li>Installez l'extension <strong>uBlock Origin</strong> (<a href="https://ublockorigin.com/" target="_blank" style="color:var(--accent);">Lien</a>).</li>
    ...
    btn.onclick = function() {
        window.open('./multiview-cleaner.user.js', '_blank');
    };
...
function loadAll(){
  ...
  getApiFirstMatches(TARGET_DATE).then(function(apiMatches) {
      ...
      return Promise.allSettled([
          ...
          if(sf){
              var optionsHtml = '<button class="btn sport-btn '+(S.sportFilter==='all'?'active':'')+'" onclick="applySportFilter(\'all\');">Tous les sports</button>';
              sportNames.forEach(function(sp){
                  if (sp !== 'EN DIRECT') {
                      var isSel = S.sportFilter === sp;
                      optionsHtml += '<button class="btn sport-btn '+(isSel?'active':'')+'" onclick="applySportFilter(\''+escJs(sp)+'\');"><span style="margin-right:4px;">'+lgFlag(sp)+'</span> '+esc(sp)+'</button>';
                  }
              });
...
function showLayoutSelector(event) {
    if (event) {
        event.stopPropagation();
        event.preventDefault();
    }

    var existing = document.getElementById('mv-layout-selector');
    if (existing) existing.remove();

    var selector = document.createElement('div');
    selector.id = 'mv-layout-selector';
    ...
    var title = document.createElement('div');
    title.style.cssText = 'font-size:11px;font-weight:bold;color:var(--muted);text-transform:uppercase;margin-bottom:4px;padding:0 4px;';
    title.textContent = 'Disposition';
    selector.appendChild(title);

    var layouts = [
        { id: 'auto', name: 'Grille Automatique', icon: '⊞' },
        { id: 'focus', name: 'Mode Focus', icon: '⭐' },
        { id: 'vertical', name: 'Empilé (Vertical)', icon: '⊟' },
        { id: 'horizontal', name: 'Côte à côte', icon: '◫' },
    ];

    layouts.forEach(function(l) {
        var btn = document.createElement('div');
        var isActive = mvLayout === l.id;
        btn.style.cssText = 'display:flex;align-items:center;gap:8px;padding:8px;border-radius:4px;cursor:pointer;transition:background 0.2s;background:' + (isActive ? 'rgba(255,255,255,0.15)' : 'transparent');

        btn.onmouseenter = function() { if(!isActive) this.style.background = 'rgba(255,255,255,0.1)'; };
        btn.onmouseleave = function() { if(!isActive) this.style.background = 'transparent'; };
        ...
        btn.innerHTML = '<span style="font-size:14px;width:20px;text-align:center;">' + l.icon + '</span><span style="font-size:13px;' + (isActive ? 'font-weight:bold;color:var(--accent);' : 'color:#fff;') + '">' + l.name + '</span>';
        selector.appendChild(btn);
    });
