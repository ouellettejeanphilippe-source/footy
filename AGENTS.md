...
- Tests : `npm test` (`node --test tests/*.test.js`, puis trois suites Playwright ; un fichier `tests/*.test.js` est pris en compte sans rien déclarer) ; `npm run test:domains` à part, parce qu'il dépend du réseau.
