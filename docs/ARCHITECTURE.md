...
`donnerLeSon(idx, raison)` est le point de décision unique, appelé à trois moments : la tuile annonce qu'elle **joue** (`video_state`), un **clic** est fait dans son cadre (`mv_frame_clicked` — une activation fraîche, et la seule sortie après un refus), ou elle devient celle qu'on regarde (`focusStream`). Le son ne va qu'à la tuile active. En face, le script utilisateur **vérifie** le résultat (`verifierLeSon`) : 400 ms après, une vidéo arrêtée est remise en muet et relancée, et l'état réel est rapporté (`sound_state`, `blocked`). L'application le retient (`noterSonBloque`, `_sonBloque`), le dit une fois, et n'insiste plus jusqu'au prochain geste — insister ferait osciller la lecture entre muet et arrêté. Réglage `son_auto`, allumé par défaut, dans le menu ⋯.
