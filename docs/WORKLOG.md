...
### 11 May 2026 - Fix matchup matching for abbreviated NHL team names like Vegas Golden Knights
- **Fichiers touchés** : `js/match.js`
- **Résumé** : Amélioration de la logique de validation croisée `isMatchPair` pour inclure la conservation des mots d'équipe originaux (raw) tout en vérifiant l'occurrence dans la chaîne adverse. De plus, pour ne pas déséquilibrer le calcul du seuil d'occurrence à 75%, ce dernier est dorénavant basé sur le décompte des mots originaux (`rawWordsCount`) à la place de la longueur du tableau combiné (`shortWords`).
- **Problèmes résolus** : Certains matchs provenant des scrapers, comme Onhockey, présentaient des noms d'équipe trop courts (par exemple 'Vegas' contre 'Golden Knights' au lieu de 'Vegas Golden Knights'). Ces derniers étaient mal identifiés suite aux correspondances d'alias plus forts (par exemple 'Vegas' renvoyait vers 'Las Vegas Raiders' NFL). En gardant et vérifiant les mots bruts et normalisés, et en basant le pourcentage du seuil de matching sur le nombre initial de mots, ces flux se connectent dorénavant correctement avec le système.
### 11 May 2026 - Amélioration du scroll automatique au chargement (scrollToNow)
