...
export function resolveStreamUrl(url) {
    return new Promise(function(resolve) {
        ...
        // Convert Twitch URLs to embed format with parent
        var twitchMatch = url.match(/(?:twitch\.tv\/|player\.twitch\.tv\/\?channel=)([^"&?\/\s]+)/i);
        if (twitchMatch && twitchMatch[1] && twitchMatch[1].toLowerCase() !== 'videos') {
            var parent = window.location.hostname || 'localhost';
            resolve('https://player.twitch.tv/?channel=' + twitchMatch[1] + '&parent=' + parent);
            return;
        }
        ...
        // Custom Deep Resolution Logic (Opt-In)
        if (typeof userPrefs !== 'undefined' && userPrefs.deepResolve) {
            lg('Deep resolve triggered for', url);
            fetchPage(url).then(function(html) {
                var doc = new DOMParser().parseFromString(html, 'text/html');
                var lowerUrl = url.toLowerCase();
                var finalSrc = null;

                // 1. Domain specific deep extractors
                if (lowerUrl.indexOf('methstreams') >= 0 || lowerUrl.indexOf('crackstreams') >= 0) {
                    // Try to find base64 encoded src or specific iframe
                    var scripts = doc.querySelectorAll('script');
                    for (var i=0; i<scripts.length; i++) {
                        var text = scripts[i].textContent || '';
                        if (text.includes('atob(')) {
                             var match = text.match(/atob\(['"]([^'"]+)['"]\)/);
                             if (match && match[1]) {
                                 try { finalSrc = atob(match[1]); break; } catch(e){}
                             }
                        }
                    }
                    if (!finalSrc) {
                        var iframe = doc.querySelector('iframe');
                        if (iframe) finalSrc = iframe.getAttribute('src');
                    }
                } else if (lowerUrl.indexOf('footybite') >= 0 || lowerUrl.indexOf('nflbite') >= 0 || lowerUrl.indexOf('mlbbite') >= 0) {
                     var els = doc.querySelectorAll('[data-stream], [data-url], [data-src]');
                     for(var i=0; i<els.length; i++) {
                         var val = els[i].getAttribute('data-stream') || els[i].getAttribute('data-url') || els[i].getAttribute('data-src');
                         if(val) {
                             if(val.startsWith('aHR0c')) {
                                 try { finalSrc = atob(val); break; } catch(e){}
                ...
                } else if (lowerUrl.indexOf('streamonsport') >= 0) {
                     var panels = doc.querySelectorAll('.player-panel');
                     for (var i=0; i<panels.length; i++) {
                         var dataEmbed = panels[i].getAttribute('data-embed');
                         if (dataEmbed) {
                ...
                } else if (lowerUrl.indexOf('streameast') >= 0) {
                     // check next.js data
                     ...
                     if (iframeMatch) {
                         try {
                             ...
                             if (iframeStreams && iframeStreams.length > 0 && iframeStreams[0].src) {
                                 finalSrc = iframeStreams[0].src;
                             }
                         } catch(e){}
                     }
                     if (!finalSrc) {
                ...
                // 2. Generic Fallback for Deep Resolve
                if (!finalSrc) {
                    var iframe = doc.querySelector('iframe');
                    if (iframe && iframe.getAttribute('src') && !iframe.getAttribute('src').includes('ads') && iframe.getAttribute('src').startsWith('http')) {
                        finalSrc = iframe.getAttribute('src');
                    }
                }
                ...
                if (finalSrc) {
                    if (typeof resolveUrl === 'function') {
                        ...
                        finalSrc = new URL(finalSrc, url).href;
                    }
                    resolve(finalSrc);
                } else {
                    resolve(url); // No iframe found, return original
                }
            }).catch(function(e) {
                resolve(url);
            });
        ...
        // Standard Hardcoded Fallback: OnHockey (when deepResolve is OFF)
